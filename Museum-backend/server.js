...
const db = new sqlite3.Database("./db.db", (err) => {
  ...
  return console.log("База данных подключена");
});
...
app.post("/add-publish", async (req, res) => {
  ...
  if (!id) return res.status(500).json({ message: "Неправильный токен" });
  db.run(
    "insert into events (date, type, name, description, price) values (?, ?, ?, ?, ?)",
    [date, type, name, description, 1000],
    function (err) {
      if (err) {
        return res
          .status(500)
          .json({ message: "Не получилось создать событие" });
      }
  ...
  db.run(
    ...
    function (err) {
      if (err) {
        return res
          .status(500)
          .json({ message: "Не получилось создать новость" });
      }
...
app.post("/send-review", async (req, res) => {
  ...
  db.run(
    "insert into review (typeOrganization, region, sex, age, overview, review) values (?, ?, ?, ?, ?, ?)",
    [typeOrganization, region, sex, age, overview, review],
    function (err) {
      if (err) {
        return res
          .status(500)
          .json({ message: "Не получилось отправить отзыв" });
      }
...
app.post("/excursion-requests", async (req, res) => {
  const { username } = req.body;
  let sql = "select * from excursion_requests";
  let params = [];

  if (username && username !== "Все") {
    sql = "select * from excursion_requests where username = ?";
    params = [username];
  }
  db.all(sql, params, (err, row) => {
    if (!row || err) {
      return res
        .status(500)
        .json({ message: "Не получилось запросить записи на экскурсии" });
    }
    res.status(200).json(row);
  });
});

app.post("/add-excursion-requests", async (req, res) => {
  const { username, email, date, duration, excursion, total, totalPrice } =
    req.body;
  db.run(
    "insert into excursion_requests (username, email, date, duration, excursion, total, totalPrice) values (?, ?, ?, ?, ?, ?, ?)",
    [username, email, date, duration, excursion, total, totalPrice],
    function (err) {
      if (err) {
        return res
          .status(500)
          .json({ message: "Не получилось отправить запись на экскурсию" });
      }
      return res
        .status(200)
        .json({ message: "Получилось отправить запись на экскурсию!" });
    }
  );
});
